	...
	FRDGTextureRef& InstancedEditorDepthTexture)
{
	...
	//SY
	const FScreenPassTexture SceneSY(SceneTextureParameters.GBufferATexture, PrimaryViewRect);
	...
	//SY
	PassSequence.SetEnabled(EPass::SY, EngineShowFlags.SYRender);
#if DEBUG_POST_PROCESS_VOLUME_ENABLE
	...
	#endif
	//SY
	if (PassSequence.IsEnabled(EPass::SY))
	{
		
		FMyTestInputs PassInputs;
		PassSequence.AcceptOverrideIfLastPass(EPass::SY, PassInputs.OverrideOutput);
		PassInputs.SceneColor = SceneSY;
		PassInputs.SceneColor01 = SceneColor;
		PassInputs.SceneColor02 = SceneDepth;
		PassInputs.SceneColor03 = Velocity;
		PassInputs.SceneColor04 = CustomDepth;
		PassInputs.SceneColor05 = BlackDummy;
		PassInputs.MyColor = FVector4(0.5, 0.5, 0.5, 0.5);

		SceneColor = AddMyTestPass(GraphBuilder, View, PassInputs);
	}
}
